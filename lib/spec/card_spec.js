(function() {
  describe('Card', function() {
    describe('attributes', function() {
      ...
      describe('level_5_ability', function() {
        return it('returns the name of the card\'s level 5 ability as a string', function() {
          var card;
          card = new Card({
            level_5_ability: "Test Ability 2"
          });
          return expect(card.level_5_ability).toEqual("Test Ability 2");
        });
      });
      describe('level_10_ability', function() {
        return it('returns the name of the card\'s level 10 ability as a string', function() {
          var card;
          card = new Card({
            level_10_ability: "Test Ability 3"
          });
          return expect(card.level_10_ability).toEqual("Test Ability 3");
        });
      });
      describe('level_15_ability', function() {
        return it('returns the name of the card\'s level 15 ability as a string', function() {
          var card;
          card = new Card({
            level_15_ability: "Test Ability 4"
          });
          return expect(card.level_15_ability).toEqual("Test Ability 4");
        });
      });
      return describe('level', function() {
    ...
    describe('properties', function() {
      ...
      return describe('effects', function() {
        return it('returns a list of effects applied to this card as an Array of strings', function() {
          var card;
          card = new Card();
          card.add_effect("test effect");
          return expect(card.effects).toEqual(["test effect"]);
        });
      });
    });
    ...
    describe('#total_attack_score', function() {
      return it("returns the card's calculated attack score as an integer", function() {
        var card;
        card = new Card({
          base_attack: 100,
          attack_per_level: 20,
        ...
        return expect(card.total_attack_score()).toEqual(140);
      });
    });
    ...
    describe('#add_effect', function() {
      return it('adds the given value to the card\'s effects list', function() {
        var card;
        card = new Card();
        card.add_effect("test effect");
        return expect(card.effects).toEqual(["test effect"]);
