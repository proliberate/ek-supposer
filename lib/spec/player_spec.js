(function() {
  describe('Player', function() {
    ...
    describe('#attack', function() {
      var card, player1, player2;
      card = {
        base_attack: 50
      };
      player1 = new Player();
      player2 = new Player();
      it('calls opposite_card.take_damage(attacking_card.total_attack_score())', function() {
        ...
        return expect(player2.take_damage).not.toHaveBeenCalled();
      });
      return it('calls opponent.take_damage(attacking_card.total_attack_score()) where there is no opposite card', function() {
        player1.field.push(new Card(card));
        player1.attack(player2);
        spyOn(player2, 'take_damage');
        player1.attack(player2);
        return expect(player2.take_damage).toHaveBeenCalledWith(50);
      });
    });
    ...
    describe('#out_of_cards', function() {
      return it("returns true if the player has no cards", function() {
        var player;
        player = new Player();
    ...
    return describe('#use_ability', function() {
      return it("calls the given Ability with the given options", function() {
        var TestAbility, player;
        player = new Player();
        TestAbility = new Ability();
        spyOn(TestAbility, 'activate');
        player.use_ability(TestAbility, 500);
        return expect(TestAbility.activate).toHaveBeenCalledWith(player, 500);
      });
    });
  });
